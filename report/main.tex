...
\customcoadvisorlabel{Co-Supervisors}
...
\maketitle
\begin{abstract}
    In the modern era, Wi-Fi devices have permeated virtually every facet of our daily lives, from homes to workplaces, and public spaces. This ubiquitous presence of Wi-Fi technology has paved the way for innovative applications beyond traditional internet connectivity.
    Wi-Fi sensing refers to a collection of systems that exploit wireless signals measurements to achieve task usually addressed by vision-based methods, such as gesture recognition, room occupation or objects localization.
    This study presents a first-of-a-kind approach to leverage the pervasive nature of Wi-Fi technology to develop a robust and non-intrusive person identification system.
    To accomplish so, a unique dataset is created, capturing Channel State Information (CSI) data as a total of 6 individuals stand between two communicating antennas. CSI data are based on Orthogonal Frequency Division Multiplexing methodology,
    that allows the gathering of amplitude and phase information of numerous subcarriers waves, resulting in a feature-rich data space. In recognition of the challenges posed by real-world environments,
    a rigorous preprocessing pipeline is employed to detect and remove outliers and smooth out noise, ensuring the reliability of the novel dataset.
    The heart of the system resides in the application of Convolutional Neural Networks (CNNs) for the subsequent classification task. CNNs, known for their prowess in computer vision, are adapted to exploit the spatial and temporal patterns inherent to CSI data.
    The experimental results underscore the efficacy of this approach, showcasing high levels of accuracy (92.4\% - 94.2\%) in a controlled environment, opening the doors to new research topics and technological deployments.
    The non-intrusive and privacy-conscious nature of the presented WiFi-based person ID system positions it as a versatile solution for a wide range of applications, encompassing security, access control, and smart home automation.
\end{abstract}
...
which plays a pivotal role in implementing wireless sensing applications. The RSSI serves as a measure of the reduction in the strength of radio signals
...
indoor localization systems \cite{RSSI_indoorloc,RSSI_indoorloc_new}, whose goal is to determine the precise location of a target within an enclosed space with meter-precision,
...
The last key component used to describe a wave is the phase. The phase of a wave refers to its position within one complete cycle, indicating how much of
...
data rate to be lower than the one required by a single stream of similar bandwidth and makes the system less susceptible to corruption.
...
A lower value of alpha gives more weight to past observations, producing a smoother forecast, while a higher value of alpha gives more weight to current observations, enabling a faster response to changes.
In this work the value is set to $\alpha = 0.8$, but further testing and tuning may be required. For what regards the initial and last values of the dataset,
those with a number of observations to their left or their right less than half of the window size, a static MAD filter is applied over the corresponding slices of size = $w/2 - 1$, and the outliers are replaced with the median.
...
with respect to other outlier detection statistical methods such as the Z-Score, which relies on the mean and the standard deviation to discern anomalies
...
when considering the LOS compared to other paths, leading to less signal fading and attenuation.
...
from the lowest subcarrier index to the highest one. This structure remains relatively consistent regardless of the selected time frame. A person between the antennas instead, introduces notable noise caused by overall amplitude modifications,
with clear differences between two distinct people. Indeed, by analyzing the data points, it becomes evident that the distribution of the amplitudes of each subcarrier for the empty room approximates quite well a Gaussian distribution, whereas the distribution of the people-related data exhibits
...
from biology and the natural world, even though they end up to be completely different from biological systems.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/convfield3.png}
    \caption{The receptive field of the units in the deeper layers of a convolutional network is larger than the one of the units in the shallow layers.
...
It can be thought as a sliding kernel that returns the statistic instead of performing the convolution inner operation. Another popular pooling function is Average Pooling, which, as the name suggests,
...
very large and complex models such as Neural Networks tend to reduce the training bias up to a point where the accuracy on training reaches 100\%, at the expense of variance that ends up being too large. This derives from the Universal Approximation Theorem, that explains how a shallow neural network has the capacity of approximating any continuous function up to any arbitrarily degree of precision, causing it to perfectly modelling the noise in the training set \cite{overfitting}.
...
tens of layers and millions of units, the paths connecting the first and last layer are so many that this probability becomes insignificant. Many variants of Dropout have been proposed \cite{dropout_variants}, such as \emph{Max Dropout} that sets the
...
Even though those properties could somewhat be obtained using the standard double-channel input image fabric, it would be academically engaging to see a performance comparison between the two ideas, but it was beyond the scope of this research.
...
\end{enumerate}
Dropout here refers to \emph{Spatial Dropout} \cite{spatial_dropout}, also called 2-D Dropout, that works by randomly deactivating not single units but an entire feature map (or channel).
...
extracted by the previous convolutions to an output class. The activation function used in all layers was the Rectified Linear Unit (ReLU), the most common choice nowadays, that boosts convergence
time and increases overall sparsity.

\chapter{Results}
This chapter summarizes the complete workflow for addressing person ID through Wi-Fi signals, then explains how the proposed model was trained, and finally the results achieved during validation phase.
\section{Working Pipeline}
The complete working pipeline for solving CSI-based person Identification task starts with the acquisition of CSI data, as seen in Fig. \ref{fig:pipeline}. The subject is required to stand between two Wi-Fi antennas that are communicating by exchanging ping packets continuously, using OFDM technology.
A two seconds window is retrieved, from which amplitude and phase information are extracted using the formulas defined in Section \ref{sec:IQ}, for each of the 52 available subcarriers. The source data is then preprocessed following the procedure of Chapter \ref{chap:preprocess}: The raw amplitude data is then filtered using the Hampel Identifier, to remove outliers, and
the series is consequently smoothed out exploiting the Discrete Wavelet Transform (DWT), to increase the Signal-To-Noise ratio; the phases are instead calibrated to remove measurements errors by the use of a linear transformation, obtaining informative features of the phases.
The two types of data are combined into a single image as seen in Section \ref{Sec:structuring}, ready to be fed to the Convolutional Neural Network model.
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{images/pipeline.png}
    \caption{The complete workflow for person ID through Wi-Fi signals.}
    \label{fig:pipeline}
\end{figure}
Lastly, the CNN presented in Section \ref{sec:CNN} elaborates the combined image, analyzing the sanitized data of 200 packets, and finally returns the appropriate person identity between all the possible choices.
\section{Training}
The whole system was designed with PYTHON 3.9.7 utilizing the Numpy and Pandas libraries, specialized in multidimensional data operations, for signal preprocessing and structuring.
The various tested Neural Network architectures were built upon the PyTorch package, and were trained and validated on a single NVIDIA Tesla T4 GPU in cloud computing, with 15 GB of VRAM.
The final model was asked to classify an incremental number of labels, starting from binary classification with just 2 people up to 6 people. When working with a number of classes less than 6,
the model was trained and tested not only once, but on a subset of size 3 of all the possible permutations of people. For instance, when dealing with 3 output classes, one could choose up to $\binom{6}{3} = 20$
different combination of people. From that set 3 triplets were sampled, and the model worked on the data of each triplet for a total of 3 independent trainings. At the end, their respective results were averaged to get a better estimation of the model's capabilities.
Likewise, the same methodology was applied for all the other number of classes. In this way, the possibility of a performance biased by the choice of the people to compare is reduced. Furthermore, the results of each combination was the outcome of a Five-Fold Cross-Validation performed with those selected classes, with the aim to obtain a score less dependent on the validation set choice.
Each training procedure was executed with the use of the Stochastic Gradient Descent (SGD) with Momentum optimizer, for a total of 200 epochs, a momentum of 0.9 and an initial learning rate \emph{lr} set to 0.001,
Additionally, a scheduler was implemented to divide the \emph{lr} by 3 at epoch 140, in order to retain a more stable and fine-grained loss exploration due to a decreased gradient update speed, over the course of the last 60 epochs.

SGD with Momentum is an improvement of the standard SGD algorithm \cite{sgd_Momentum}, used to dampen oscillations in the loss function, accelerate convergence and hopefully skip local minima. It helps the optimization algorithm to gain speed in directions where the gradient consistently points, increasing stability during the parameter space navigation.
Mathematically, momentum is introduced by maintaining a moving average of past gradients. The momentum term, denoted as $\beta$ (usually a value close to 0.9), is a hyperparameter that controls the influence of past gradients. A high $\beta$ value gives more weight to past gradients, while a low value allows for more responsiveness to current gradients. An update step is in fact described by:
$$\boldsymbol{V}_t = \beta\boldsymbol{V}_{t-1} + (1 - \beta)\nabla_\theta J(\boldsymbol{\theta}, X, \boldsymbol{y})$$
$$\boldsymbol{\theta} \leftarrow \boldsymbol{\theta} - \lambda  \boldsymbol{V}_t$$
Where $\boldsymbol{V}_t$ holds the momentum vector at time t, $\boldsymbol{\theta}$ are the parameters to update, $\lambda$ is the learning rate and $J$ the loss function. Specifically, the function used for training was the Cross-Entropy loss with the addition to class weights to compensate a slight samples imbalance, where each weight was just the number of samples for class $i$ divided by the total number of samples.
As usual, the Soft Max function was applied to the output activations, to transform them into a probability distribution right before computing the loss.
\section{Validation results}
As explained before, apart from the 6 output classes case, the presented results are the averages of a 5-fold cross-validation on 3 different combination of people. For example, the numbers shown for the 2-classes situation, is the average of cross-validated model performances on (P1, P3), (P2, P4), (P5, P6), where PN indicates Person N. Testing all possible combination would be better, but also computationally expensive and time-consuming.
For every task, the following common classification metrics were calculated:
$$ Accuracy = \frac{correct}{total} \qquad Precision = \frac{TP}{TP + FP} \qquad Recall = \frac{TP}{TP + FN}$$
where $TP, TN, FP, FN$ stand for the number of true positive, true negative, false positive and false negative predictions. Since the task is a multiclass problem, each value (apart from accuracy) is the mean over each single class metric.
\\
...
Fig. \ref{fig:loss} shows how the average training loss changes epochs after epochs in comparison to the validation loss, when dealing with 6 outputs classes, suggesting how the number of epochs are optimal for the given dataset and task.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{images/train_val_loss.png}
    \caption{The train (blue) and validation (orange) loss value over 180 epochs.}
    \label{fig:loss}
\end{figure}
\chapter*{Conclusion}
The presented work serves as a foundational stepping stone for venturing into the fascinating realm of Wi-Fi sensing for person identification. The landscape of publicly available literature on this subject is, at present, close to desert, emphasizing the significance of the proposed method
as an essential reference point for tackling this complex problem. A great number of hyperparameters could be tested, starting from the preprocessing phase of the data up to model's layers, regularization and training. The assembled dataset is brand new, and future endeavors could significantly
enhance this collection by incorporating a more extensive range of subjects, capturing diverse body poses and orientations, a wider variety of clothing and personal accessories, and even introducing variations in background environments, such as different rooms, furnishings, open spaces, and outdoor scenarios.
Even the hardware setting could be improved adding multiple antennas to surround the subject, in a MIMO (Multiple Input Multiple Output) scenario, retrieving a larger amount of signals coming from different directions that have been subjected to unique modifications.
Despite this, the performance of the designed working pipeline is excellent, solving the classification task with high accuracy, demonstrating how the proposed system could be thought as a baseline model for addressing such a task; The continuous evolution of Deep Learning approaches and the advancements in
radio communication technology will likely open the future of Wi-Fi sensing to new horizons, with new challenges and amazing solutions waiting to be discovered.

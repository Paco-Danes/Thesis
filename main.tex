...
Where $i$ indexes the samples in the first half of the window, $s_i$ is the smoothed value and $x_i$ is the observation.
Moreover, the Hampel Identifier doesn't pose any strong assumption on the distribution of the data and works well even with non-normally distributed points. This is a clear distinction
...
$k_i$ is the subcarrier index, ranging from -26 to +26 and N is a parameter found in the IEEE 802.11 specifications.
...
Subtracting the linear term $ak_i + b$ from the raw phase $\hat{\phi}_i$, we obtain a linear combination of true phases, denoted as $\Phi_i$, from which the random phase offsets have been removed (omitting the small measurement noise Z):
...
\begin{figure}[h]
    ...
    \caption{Raw and sanitized phases of a single subcarrier over 100 consecutive pakets.}
    \label{fig:phase_cal}
\end{figure}

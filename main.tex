...
different combination of people. From that set 3 triplets were sampled, and the model worked on the data of each triplet for a total of 3 independent trainings. At the end, their respective results were averaged to get a better estimation of the model capabilities.
...
where $TP, TN, FP, FN$ stand for the number of true positive, true negative, false positive and false negative prediction. Since the task is a multiclass problem, each value (apart from accuracy) is the mean over each single class metric.
\\
...
Despite this, the performance of the designed working pipeline is great, solving the classification task with high accuracy, demonstrating how the proposed system could be thought as a baseline model for addressing such a task; The continuous evolution of Deep Learning approaches and the advancements in

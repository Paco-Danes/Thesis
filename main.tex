...
Each training procedure was executed with the use of the Stochastic Gradient Descent (SGD) with Momentum optimizer, for a total of 180 epochs, a momentum of 0.9 and an initial learning rate \emph{lr} set to 0.001,
Additionally, a scheduler was implemented to divide the \emph{lr} by 2 at epoch 140, in order to retain a more stable and fine-grained loss exploration due to a decreased gradient update speed, over the course of the last 40 epochs.

SGD with Momentum is an improvement of the standard SGD algorithm, used to dampen oscillations in the loss function, accelerate convergence and hopefully skip local minima. It helps the optimization algorithm to gain speed in directions where the gradient consistently points, increasing stability during the parameter space navigation.
